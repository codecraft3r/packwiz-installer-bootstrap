...
object UpdateManager {
    ...
    private fun performSelfUpdate(release: GitHubRelease) {
        ...
        replaceSelf(currentPath, newFile)
    }

    private fun replaceSelf(currentFile: File, newFile: File) {
        val osName = System.getProperty("os.name").lowercase()
        val script = if (osName.contains("win")) {
            createWindowsUpdater(currentFile, newFile)
        } else {
        ...
        ProcessBuilder(script.absolutePath, currentFile.absolutePath, newFile.absolutePath)
            .inheritIO()
            .start()
    ...
    private fun createWindowsUpdater(current: File, newFile: File): File {
